...
for rep_iter in range(pms.max_repeat):
    ...
    if rank == 0:
        ...
        split_X_tr = np.array_split(X_train, [pms.train_num, 2*pms.train_num, 3*pms.train_num, 4*pms.train_num, 5*pms.train_num], axis=0)
        split_Y_tr = np.array_split(Y_train, [pms.train_num, 2*pms.train_num, 3*pms.train_num, 4*pms.train_num, 5*pms.train_num], axis=0)
    else:
        split_X_tr = None
        split_Y_tr = None
...
for rep_iter in range(pms.max_repeat):
    ...
    for nei_iter_1 in range(norm_N_j):
        ...
        for nei_iter_2 in range(norm_N_j):
            ...
            K_mat_j[index_1][index_2] = centralize_kernel(tmp_kernel)  # (K_mat_j_subset)_{i,j} = <z(X_i,sub), z(X_j,sub)>

    alpha_ini, lam_ini = solve_global_svd(K_mat_j[rank][rank], pms.target_k)
    K_mat_j[rank][rank] = K_mat_j[rank][rank] + pms.ill_thres * np.min(lam_ini) / pms.local_n \
                                 * np.ones((pms.local_n, pms.local_n))
    ...
    for ii in range(pms.target_k):
        ...
        acc_DeKPCA[0, rep_iter] += np.abs(v1.T @ kernel_total_j @ v2 / np.sqrt(v1.T @ K_mat_j[rank][rank] @ v1) \
                                       / np.sqrt(v2.T @ kernel_tt @ v2))
    # -----(End) DeKPCA-----
...
for rep_iter in range(pms.max_repeat):
    ...
    toc_DeKPCA_Ny_init = MPI.Wtime()
    time_DeKPCA_Ny_j_init = toc_DeKPCA_Ny_init - tic_DeKPCA_Ny_init
    # ------------
    ...
    for k_iter in range(pms.target_k):
        ...
        for ADMM_iter in range(10):
            ...
            # DeKPCA.3.3 Update and boardcast \eta_j.
            if z_flag == 1 and alpha_flag == 1:
            # if ADMM_iter == 2 or ADMM_iter == 4 or ADMM_iter == 6 or ADMM_iter == 8:
                eta_old = np.array(eta_j)
                # Edit for DeKPCA-Ny
                # eta_j = eta_j + multi_dot([alpha_j, E_j, np.diag(rho.flatten())]) \
                #         - multi_dot([inv_K_mat_j[rank], phi_z_xi, np.diag(rho.flatten())])
    ...
    toc_DeKPCA_Ny_cal = MPI.Wtime()
    time_DeKPCA_Ny_j_cal = toc_DeKPCA_Ny_cal - tic_DeKPCA_Ny_cal
    ...
    for ii in range(pms.target_k):
        ...
        v2 = alpha_ggt[:, ii].reshape(-1, 1)
        acc_DeKPCA_Ny[0, rep_iter] += np.abs(v1.T @ kernel_total_j_subset @ v2 / np.sqrt(v1.T @ K_mat_j_subset[rank][rank] @ v1) \
                                       / np.sqrt(v2.T @ kernel_tt @ v2))
    # -----(End) DeKPCA_Ny-----

result_time_local_init = comm.gather(time_local_j_init, root=0)
result_time_local_cal = comm.gather(time_local_j_cal, root=0)
result_time_DeKPCA_Ny_init = comm.gather(time_DeKPCA_Ny_j_init, root=0)
result_time_DeKPCA_Ny_cal = comm.gather(time_DeKPCA_Ny_j_cal, root=0)
...
if rank == 0:
    ...
    list_time_DeKPCA_Ny_init = []
    list_time_DeKPCA_Ny_cal = []
    ...
    for i in range(pms.worker_num):
        ...
        list_time_DeKPCA_Ny_init.append(result_time_DeKPCA_Ny_init[i])
        list_time_DeKPCA_Ny_cal.append(result_time_DeKPCA_Ny_cal[i])
...
if rank == 0:
    ...
    print('DeKPCA-Ny init time: ', list_time_DeKPCA_Ny_init)
    print('DeKPCA-Ny cal time: ', list_time_DeKPCA_Ny_cal)
    print('local acc: ', list_acc_ini)
    print('DeKPCA acc: ', list_acc_DeKPCA)
    print('DeKPCA-Ny acc: ', list_acc_DeKPCA_Ny)
...
if rank == 0:
    # print('(Fully) local init time: ', list_time_local_init)
    # print('(Fully) local cal time: ', list_time_local_cal)
    print('(Fully) DeKPCA init time: ', list_time_DeKPCA_init)
    print('(Fully) DeKPCA cal time: ', list_time_DeKPCA_cal)
    print('(Fully) DeKPCA-Ny init time: ', list_time_DeKPCA_Ny_init)
    print('(Fully) DeKPCA-Ny cal time: ', list_time_DeKPCA_Ny_cal)
    # print('(Fully) local acc: ', list_acc_ini)
    print('(Fully) DeKPCA acc: ', list_acc_DeKPCA)
    print('(Fully) DeKPCA-Ny acc: ', list_acc_DeKPCA_Ny)