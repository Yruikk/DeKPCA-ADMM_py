...
for rep_iter in range(pms.max_repeat):
    ...
    if rank == 0:
        ...
        split_X_tr = np.array_split(X_train, [2200, 4400, 6600, 8800, 9000, 11200, 13400, 15600, 17800], axis=0)
        split_Y_tr = np.array_split(Y_train, [2200, 4400, 6600, 8800, 9000, 11200, 13400, 15600, 17800], axis=0)
    else:
        split_X_tr = None
        split_Y_tr = None
        # X_test = None
    ...
    for k_iter in range(pms.target_k):
        ...
        for ADMM_iter in range(60):
            ...
            for nei_iter in range(norm_N_j):
                nei_tmp = N_j[nei_iter, 0]
                tmp_sum += np.dot(K_mat_j[rank][nei_tmp],
                                  (-lam[nei_tmp][rank] + rho[0, nei_tmp] * zeta_nei[rank][nei_iter][rank]).T) \
            ...
            if prime_flag >= 4:
                ...
                for nei_iter in range(norm_N_j):
                    nei_tmp = N_j[nei_iter, 0]
                    lam[rank][nei_tmp] += rho[0, nei_tmp] * 0.5 * (
                                np.dot(alpha_j.T, K_mat_j[rank][rank]) - zeta[nei_tmp])
                    lam[nei_tmp][rank] += rho[0, nei_tmp] * 0.5 * (
                                np.dot(alpha_j.T, K_mat_j[rank][nei_tmp]) - zeta_nei[rank][nei_iter][rank])
                prime_flag = 0
    ...
    for ii in range(pms.target_k):
        ...
        acc_JMLR[0, rep_iter] += (v1.T @ kernel_total_j @ kernel_total_j.T @ v1) / (v1.T @ K_mat_j[rank][rank] @ v1)
    # -----(End) JMLR 2010-----
...
if rank == 0:
    with open("expA2.2Twitter_result_time2.csv", "w", newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open("expA2.2Twitter_result_acc2.csv", "w", newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerow(["Init:"])
