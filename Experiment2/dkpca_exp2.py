...
result_algo = np.zeros((pms.max_repeat, pms.iterations))
result_algo_j = np.zeros((pms.max_repeat, pms.iterations))
result_ini = np.zeros((pms.max_repeat, pms.iterations))
result_ini_j = np.zeros((pms.max_repeat, pms.iterations))
result_nei = np.zeros((pms.max_repeat, pms.iterations))
result_nei_j = np.zeros((pms.max_repeat, pms.iterations))
time_algo = np.zeros((pms.max_repeat, pms.iterations))
time_cen = np.zeros((pms.max_repeat, pms.iterations))
time_cen_comm = np.zeros((pms.max_repeat, pms.iterations))
time_cen_cal = np.zeros((pms.max_repeat, pms.iterations))
for out_iter in range(pms.max_repeat):
    time_cen_flag = True  # !!!
    for in_iter in range(pms.iterations):
        data_num = 40 * (in_iter + 1)
        ...
        if rank == 0:
            ...
            data0 = data0[0:int(pms.worker_num * data_num / 4), :]
            data3 = data3[0:int(pms.worker_num * data_num / 4), :]
            data5 = data5[0:int(pms.worker_num * data_num / 4), :]
            data8 = data8[0:int(pms.worker_num * data_num/ 4), :]
        ...
        for i in range(pms.worker_num):
            if rank in j_list[i]:
                local_n_nei[i] = comm_j[i].gather(data_num, root=0)
        ...
        K_mat_j[rank][rank] = K_mat_j[rank][rank] + pms.ill_thres * ss / data_num \
        ...
        eta_j = np.zeros((data_num, norm_N_j))
        ...
        for ADMM_iter in range(20):
            ...
            H_alpha = -2 * np.dot(K_mat_j[rank][rank], K_mat_j[rank][rank]) \
                      + multi_dot([E_j, np.diag(rho.flatten()), E_j.T])[0][0] * np.eye(data_num)
                ...
                if sum_update_flag < pms.worker_num * stop_flag:
                    ...
                    if stage == 4:
                        for i in range(pms.iterations - ADMM_iter):
                            result_algo[out_iter][ADMM_iter + i] = result_algo[out_iter][ADMM_iter - 1]
                        if rank == 0:
        ...
        time_algo[out_iter][in_iter] = time_all
        ...
        if time_cen_flag:  # !!!
            ...
            if rank == 0:
                ...
                e_cal = MPI.Wtime()
                time_cen_comm[out_iter][in_iter] = time_tmp - s_comm
                time_cen_cal[out_iter][in_iter] = e_cal - time_tmp
                time_cen[out_iter][in_iter] = e_cal - s_comm
            else:
                all_data = None
                alpha_ggt = None
                K_X_X = None
            ...
            if rank == 0:
                if e_cal - s_comm > pms.time_cen_max:  # !!!
                    time_cen_flag = False
            time_cen_flag = comm.bcast(time_cen_flag, root=0)
            ...
            result_algo_j[out_iter][in_iter] = quality_algo_j
            quality_algo = comm.allreduce(quality_algo_j, op=MPI.SUM)
            quality_algo = quality_algo / pms.worker_num
            result_algo[out_iter][in_iter] = quality_algo
        if time_cen_flag == False:
            break
...
if rank == 0:
    with open("exp2_result_4.csv", "w", newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerow(["ini", "nei"])
        f_csv.writerow(["40", "80", "120", "160", "200", "240", "280"])
        f_csv.writerow(["ini:"])
        f_csv.writerow([np.mean(result_ini[:, 0]), np.mean(result_ini[:, 1]), np.mean(result_ini[:, 2]),
                        np.mean(result_ini[:, 3]), np.mean(result_ini[:, 4]), np.mean(result_ini[:, 5]),
                        np.mean(result_ini[:, 6])])
        f_csv.writerow(["nei:"])
        f_csv.writerow([np.mean(result_nei[:, 0]), np.mean(result_nei[:, 1]), np.mean(result_nei[:, 2]),
                        np.mean(result_nei[:, 3]), np.mean(result_nei[:, 4]), np.mean(result_nei[:, 5]),
                        np.mean(result_nei[:, 6])])
        f_csv.writerow(["algo:"])
        f_csv.writerow([np.mean(result_algo[:, 0]), np.mean(result_algo[:, 1]), np.mean(result_algo[:, 2]),
                        np.mean(result_algo[:, 3]), np.mean(result_algo[:, 4]), np.mean(result_algo[:, 5]),
                        np.mean(result_algo[:, 6])])
        f_csv.writerow(["time_algo:"])
        f_csv.writerow([np.mean(time_algo[:, 0]), np.mean(time_algo[:, 1]), np.mean(time_algo[:, 2]),
                        np.mean(time_algo[:, 3]), np.mean(time_algo[:, 4]), np.mean(time_algo[:, 5]),
                        np.mean(time_algo[:, 6])])
        f_csv.writerow(["time_cen:"])
        f_csv.writerow([np.mean(time_cen[:, 0]), np.mean(time_cen[:, 1]), np.mean(time_cen[:, 2]),
                        np.mean(time_cen[:, 3]), np.mean(time_cen[:, 4]), np.mean(time_cen[:, 5]),
                        np.mean(time_cen[:, 6])])
        f_csv.writerow(["time_cen_comm:"])
        f_csv.writerow([np.mean(time_cen_comm[:, 0]), np.mean(time_cen_comm[:, 1]), np.mean(time_cen_comm[:, 2]),
                        np.mean(time_cen_comm[:, 3]), np.mean(time_cen_comm[:, 4]), np.mean(time_cen_comm[:, 5]),
                        np.mean(time_cen_comm[:, 6])])
        f_csv.writerow(["time_cen_cal:"])
        f_csv.writerow([np.mean(time_cen_cal[:, 0]), np.mean(time_cen_cal[:, 1]), np.mean(time_cen_cal[:, 2]),
                        np.mean(time_cen_cal[:, 3]), np.mean(time_cen_cal[:, 4]), np.mean(time_cen_cal[:, 5]),
                        np.mean(time_cen_cal[:, 6])])
